...
- **图形化设置窗口（分两个选项卡）**
  - **总设置**：签名、默认样式、品牌Logo、边框背景图、JPEG质量/采样、输出选项、智能样式等全局设置
  - **样式设置**：半透明水印、白条边框、纯色边框各自的参数单独调整
  - 选择窗口里点「设置...」，所有配置都能在窗口里改，保存后立即生效；右侧内嵌实时预览，调参即时看效果
  - 设置保存在 `水印设置.ini` 中，无需手动编辑文件

- **实时预览**
  - **选择窗口右侧**实时预览，支持两种视图：**全部视图**（2×2 缩略图网格同时显示全部勾选样式，默认）和**单个视图**（单个大图，可下拉切换样式）
  - 勾选/取消样式、改品牌Logo、换照片即时刷新；未选照片时用 `input/` 文件夹里的照片示例展示效果
  - **设置窗口右侧内嵌实时预览**：拖动滑杆（边框高度/Logo大小/透明度/字体/背景图…）、改颜色/位置/品牌/签名，预览即时更新，调好再保存；顶部下拉可切换预览的样式
  - **设置窗口有「恢复默认」按钮**：一键把全部设置重置为出厂默认值（预览立即更新，点保存后生效）
